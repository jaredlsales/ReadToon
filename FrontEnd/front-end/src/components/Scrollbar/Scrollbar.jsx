...
const Scrollbar = ({ capitulos, slug }) => {
  
  // Pegamos o primeiro e o último com segurança para os cards roxos
  const primeiroCap = capitulos && capitulos.length > 0 ? capitulos[capitulos.length - 1] : null;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Link to={`/NanoMachine/${slug}/${primeiroCap?.numero}`} className="bg-[#8E3FDD] p-6 rounded-md text-center border border-white/5 shadow-lg hover:brightness-110 transition-all">
          <span className="block text-xs uppercase text-white/70 font-bold mb-1 tracking-wider">First Chapter</span>
          <span className="text-2xl font-bold block ">Chapter {primeiroCap?.numero || '1'}</span>
        </Link>

        {/* Card do Último Capítulo (Latest) */}
        <Link to={`/NanoMachine/${slug}/${ultimoCap?.numero}`} className="bg-[#8E3FDD] p-6 rounded-md text-center border border-white/5 shadow-lg hover:brightness-110 transition-all">
          <span className="block text-xs uppercase text-white/70 font-bold mb-1 tracking-wider">Latest Release</span>
          <span className="text-2xl font-bold block ">Chapter {ultimoCap?.numero || '---'}</span>
        </Link>
      </div>
