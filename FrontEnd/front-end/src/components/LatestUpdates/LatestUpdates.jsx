...
export default function LatestUpdates() {
  ...
  const getReadPath = (name, chapterNum) => {
    const slug = getSlug(name);
    if (name === "Nano Machine") return `/Read/NanoMachineChapter/${slug}/${chapterNum}`;
    if (name === "Infinite Mage") return `/Read/InfiniteMageChapter/${slug}/${chapterNum}`;
    if (name === "Star-Embracing Swordmaster") return `/Read/StarEnbracingSwordmasterChapter/${slug}/${chapterNum}`;
    if (name === "Revenge of the Iron-Blooded Sword Hound") return `/Read/RevendeOfTheIronChapter/${slug}/${chapterNum}`;
    if (name === "The Regressed Mercenary's Machinations") return `/Read/TheRegressedMercenaryChapter/${slug}/${chapterNum}`;
  ...
  return (
    <div className="bg-[#222222] text-white font-sans rounded-[3px] border border-[#312f40]">
      <div className="flex justify-between items-center p-4 border-b border-[#312f40]">
        <h3 className="text-[15px] font-semibold flex items-center gap-2">Latest Updates</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2">
        {manhwas.map((m) => {
          ...
          return (
            <div key={m.id} className="p-4 border-b border-[#312f40] flex gap-4 group">
