...
export default function StarEnbracingSwordmasterChapter() {
    ...
    async function loadData() {
      try {
        ...
        const dadosManhwa = responseManhwa.data.find(
          (m) => m.titulo === "Star-Embracing Swordmaster",
        );
    ...
    <div className="min-h-screen bg-[#111] text-white">
      <main className="max-w-[1220px] mx-auto pt-6 px-4 pb-20">
        <div className="space-y-6">
          <TopBar
            manhwaTitle={`${manhwa?.titulo || "Star-Embracing Swordmaster"} - Chapter ${chapterNumber}`}
          />

          <NavBarChapter
            capitulos={capitulos}
            capituloAtual={chapterNumber}
            slug={slug || "star-embracing-swordmaster"}
          />
